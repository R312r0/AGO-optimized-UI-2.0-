...
import { MINT_REDEEM_KEY, DEX_ADDRESESS, LOADER_INDICATOR_LOCAL, MAX_INT } from "../../../constants";
...
const TradingMarket = ({ pool }) => {
    ...
    const { contracts, balances, tokens, theme, setIsWalletModal, changeTokenBalance, approveModal, setApproveModal, setApproveDataForModal } = useSystemContext();
    ...
    const checkAllowance = async () => {
        ...
        setToken0Allowance((await tok0.methods.allowance(account, pool?.isQuickSwapPool ? DEX_ADDRESESS.QUICK_ROUTER : DEX_ADDRESESS.ROUTER).call()).length === MAX_INT.length)
        setToken1Allowance((await tok1.methods.allowance(account, pool?.isQuickSwapPool ? DEX_ADDRESESS.QUICK_ROUTER : DEX_ADDRESESS.ROUTER).call()).length === MAX_INT.length)
    ...
    const handleApprove = async () => {
        ...
        setApproveDataForModal({
            destination: pool?.isQuickSwapPool ? DEX_ADDRESESS.QUICK_ROUTER : DEX_ADDRESESS.ROUTER,
            approves: [
        ...
        try {
            ...
            if (pool?.isQuickSwapPool) {
                if (token0.symbol === "MATIC") {
                    ...
                    await contracts.QUICKSWAP_ROUTER.methods.swapExactETHForTokens(
                        0,
                        [token0.id, token1.id],
                        account,
                    ...
                    await contracts.QUICKSWAP_ROUTER.methods.swapTokensForExactETH(
                    ...
                    await contracts.QUICKSWAP_ROUTER.methods.swapExactTokensForTokens(
            ...
            message.success({ content: "Swap is done!", className: "ant-argano-message", key: MINT_REDEEM_KEY, duration: 5 });
    ...
    return (
        <div className={`trading-wrapper-exchange trading-window-box ${pool?.isQuickSwapPool ? " quick-swap-trade" : ""}`}>
            <div className='trading-wrapper-exchange__header'>
                <h1> {pool?.isQuickSwapPool ? "QuickSwap" : ""} Market  {pool?.isQuickSwapPool ? <TokenIcon iconName={"QUICK"} /> : ""}  </h1>
            ...
            {token0 && token1 ?
                <>
                    ...
                    <div className="trading-wrapper-exchange__swap-input">
                        <div className={`trading-wrapper-exchange__swap-input__header ${token0.symbol === "WMATIC" || token0.symbol === "MATIC" ? "matic-header" : ""}`}>
                            <h3> {token0.symbol} </h3>
                            {token0.symbol === "WMATIC" || token0.symbol === "MATIC" ?
                                <button onClick={() => token0.symbol === "WMATIC" ? setToken0({ id: token0.id, priceUSD: token0.priceUSD, symbol: "MATIC", name: "MAITC" }) : setToken0({ id: token0.id, priceUSD: token0.priceUSD, symbol: "WMATIC", name: "WMAITC" })}><img src={wmatic_for_matic} alt="change" /></button>
                                :
                                null
                            }
                    ...
                    <div className="trading-wrapper-exchange__swap-input">
                        <div className={`trading-wrapper-exchange__swap-input__header ${token1.symbol === "WMATIC" || token1.symbol === "MATIC" ? "matic-header" : ""}`}>
                            <h3> {token1.symbol} </h3>

                            {token1.symbol === "WMATIC" || token1.symbol === "MATIC" ?
                                <button onClick={() => token1.symbol === "WMATIC" ? setToken1({ id: token1.id, priceUSD: token1.priceUSD, symbol: "MATIC", name: "MAITC" }) : setToken1({ id: token1.id, priceUSD: token1.priceUSD, symbol: "WMATIC", name: "WMAITC" })}><img src={wmatic_for_matic} alt="change" /></button>
                                :
                                null
                            }
