...
export const LiquidityPools = () => {
    ...
    const prepareData = async (pools) => {
        ...
        setEarnPools(earnAgo.filter(item => item.address !== "0x7cbdc04ab8defbf88ccfa331e57398fb36bae595" && item.address !== agobtcWbtcPool && item.address !== agousdUsdtPool));
        setEarnAgoPools(earnAgo.filter(item => item.address === agobtcWbtcPool || item.address === agousdUsdtPool));
            ...
            {!account ?
                ...
                <>
                    <div className={`luqidity-pools-wrapper ${theme === "light" ? " luqidity-pools-wrapper-light" : ""}`}>
                        ...
                        <h3 className='luqidity-pools-wrapper-heading'> Earning AGOy </h3>
                        ...
                        <ul className='luqidity-pools-wrapper-list earn-agoy'>
                            {poolsPreparing ? <Spin size="large" indicator={LOADER_INDICATOR_LOCAL} /> :
                                <>
                                    {earnAgoPoolsFormatted.map((item) => {
                                        return (
                                            <LiquidityPoolsItem earnGovToken={true} pool={item} />
                                        )
                                    })}
                                </>
                            }
                        </ul>
                        <h3 className='luqidity-pools-wrapper-heading'> Earning CNUSD/CNBTC </h3>
                        <div className='luqidity-pools-wrapper__list-header'>
                            <h5> Pool </h5>
                            <h5>
                                Liquidity
